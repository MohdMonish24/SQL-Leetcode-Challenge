...
To find the confirmation rate of each user, we left join the Signups and Confirmations tables on user_id. 
Then, we sum the 'confirmed' actions and divide by the total actions for each user, rounding to two decimal places.
If a user didn't request any confirmations, we set their rate to 0.00 using IFNULL.
The results are grouped by user_id. This approach accurately computes the confirmation rate, handling both confirmed and timed-out messages.
